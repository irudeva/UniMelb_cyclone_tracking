      Program mapx
c
c * Original Author: Ross Murray
c
c * Notes: Modified by Kevin Keay for Linux (g77) (11/10/2003)
c
      ...
      character*80 fields,optarg,wd(3)
      integer optind,ftell
...
c-----------------------------------------------------------------------
      ...
      if(iargc().eq.0)then
        write(*,*)'Usage: mapx [-aAbBcCeEdfhHijklLmMnNopqrRsStTuv',
     * 'VwxXzZ]'
        write(*,*)'[-Fmapnumber] [-1 outfile1 [-2 outfile2]] infile1',
     * ' [infile2]'
        write(*,*)'       mapx --help'
        stop
      else
 ...
 10   continue
   ...
   12       continue
          ...
          if (char(nopt).eq.'I') then
            call getwds(optarg,3,wd,nwd,ie)
c           write (6,*) 'nwd = ',nwd
c           write (6,'(a,i3,x,a,a,a)') 
c    *       ('nwd=',iwd,'"',wd(iwd),'"',iwd=1,nwd)
            if (ie.gt.0) stop 'error in getwds'
            read (wd(1),*) icstrt
c           write (6,*) icstrt
            if (nwd.ge.2) read (wd(2),'(i10)') icstop
c           write (6,*) icstop
            head = ''
            if (nwd.ge.3) read (wd(3),'(a)') head
c           write (6,*) head(1:lnblnk(head))
          endif
...
cc              ifs = fseek(1,where(mext(iext)),0)
cc              if (ifs.ne.0) stop ' fseek unsuccessful.'
              call fseek(1,where(mext(iext)),0)  ! g77
...
c                --------------
     ...
     *       file3(1:lnblnk(file3)),'): '
          ...
          if (icstrt.ne.0) then
              head3 = head1
              head4 = head2
              if (lnblnk(head).eq.0) read (5,30) head
              head3(icstrt:icstop) = head(1:lnblnk(head))
              if (out2det) head4(icstrt:icstop) = head(1:lnblnk(head))
          else
              read (5,30) head3
              lnb = lnblnk(head3)
